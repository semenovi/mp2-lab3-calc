#include "calc.h"
int preprocessor::check_brackets(){
	int c = 0, last = 0;
...
void preprocessor::unary_minus_to_binary()
{
	if (input[0] == '-')
	{
...
void preprocessor::delete_spaces()
{
	for (int i = 0; i < input.length(); i++)
	{
		if (input[i] == ' ')
...
queue preprocessor::create_postfix()
{
...
queue preprocessor::create_prefix()
{
...
preprocessor::preprocessor(std::string _input, functions _fts)
{
	input = _input;
...
state calc::get_result_by_custom_in(state st)
{
